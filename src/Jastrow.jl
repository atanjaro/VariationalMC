...
"""
    ...
    update_fermionic_Tvec!( local_acceptance::LocalAcceptance, jastrow::Jastrow, model_geometry::ModelGeometry )

Updates elements Tᵢ of the vector T after a Metropolis update.

"""
function update_fermionic_Tvec!(isite, jsite, spin, jastrow::Jastrow, model_geometry::ModelGeometry)
    ...
    for i in 1:N
        if dims == 1
            reduced_isite_index = reduce_index_1d(i, isite, model_geometry)
            reduced_jsite_index = reduce_index_1d(i, jsite, model_geometry)
        elseif dims == 2
            reduced_isite_index = reduce_index_2d(i, isite, model_geometry)
            reduced_jsite_index = reduce_index_2d(i, jsite, model_geometry)
        end

        if haskey(jpar_map, reduced_jsite_index) || haskey(jpar_map, reduced_isite_index)
            (_, value1) = jpar_map[reduced_jsite_index]
            vᵢⱼ = value1

            (_, value2) = jpar_map[reduced_isite_index]
            vₗᵢ = value2

            Tvec_f_diff[i] = vᵢⱼ - vₗᵢ
        end
    end
    ...
    return Tvec_f_updated
end


"""
    get_fermionic_jastrow_ratio( isite::Int, jsite::Int, jastrow::Jastrow, 
                                        pht::Bool, spin::Int, model_geometry::ModelGeometry )
...
"""
function get_fermionic_jastrow_ratio(isite::Int, jsite::Int, jastrow::Jastrow, 
                                        pht::Bool, spin, model_geometry::ModelGeometry)
    ...
    # obtain elements 
    if dims == 1
        red_idx_jj = reduce_index_1d(jsite-1, jsite-1, model_geometry)
    elseif dims == 2
        red_idx_jj = reduce_index_2d(jsite-1, jsite-1, model_geometry)
    end

    if haskey(jpar_map, red_idx_jj)
        (_,  vⱼⱼ) = jpar_map[red_idx_jj]
    end

    if dims == 1
        red_idx_ji = reduce_index_1d(jsite-1, isite-1, model_geometry)
    elseif dims == 2
        red_idx_ji = reduce_index_2d(jsite-1, isite-1, model_geometry)
    end

    if haskey(jpar_map, red_idx_ji)
        (_,  vⱼᵢ) = jpar_map[red_idx_ji]
    end
    ...
    if pht
        if spin == -1    
            jas_ratio_f = exp( (Tⱼ_f - Tᵢ_f) + vⱼⱼ - vⱼᵢ )
        else
            jas_ratio_f = exp( -(Tⱼ_f - Tᵢ_f) + vⱼⱼ- vⱼᵢ )
...
function build_jastrow_factor(jastrow_type::String, model_geometry::ModelGeometry, 
    ...
    if jastrow_type == "e-den-den" || jastrow_type == "e-spn-spn"
        ...
        # generate T vector
        init_Tvec_f = get_fermionic_Tvec(jastrow_type, jpar_map, pconfig, pht, model_geometry);
...
function build_jastrow_factor(jastrow_type::String, model_geometry::ModelGeometry, 
    ...
    elseif jastrow_type == "eph-den-den" || jastrow_type == "ph-den-den"
        # generate T vectors
        init_Tvec_f = get_fermionic_Tvec(jastrow_type, jpar_map, pconfig, pht, model_geometry)
        init_Tvec_b = get_phononic_Tvec(jastrow_type, jpar_map, pconfig, pht, model_geometry)
    end
...
"""
    recalc_fermionic_Tvec!(Tᵤ::Vector{AbstractFloat}, δT::AbstractFloat)
...
"""
function recalc_fermionic_Tvec!(jastrow::Jastrow, δT::Float64)
    if debug
    ...
    Tᵣ_f = get_fermionic_Tvec(jastrow_type, jpar_map, pconfig, pht, model_geometry)
